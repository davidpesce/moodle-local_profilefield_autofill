...
class helper {
    ...
    public static function display_grouped_mappings($groups, $pageurl) {
        ...
        foreach ($groups as $group) {
            ...
            $headertext = $targetname . ' → "' . format_string($group['targetvalue'])
            ...
            echo \html_writer::end_div(); // End btn-group.
            ...
            foreach ($group['mappings'] as $mapping) {
                ...
                $toggleurl = new \moodle_url(
                    $pageurl,
                    ['action' => 'toggle', 'id' => $mapping->id, 'grouped' => 1, 'sesskey' => sesskey()]
                );
                echo \html_writer::link(
                    $toggleurl,
                ...
                $deleteurl = new \moodle_url(
                    $pageurl,
                    ['action' => 'delete', 'id' => $mapping->id, 'grouped' => 1, 'sesskey' => sesskey()]
                );
                echo \html_writer::link(
                    $deleteurl,
            ...
            echo \html_writer::end_div(); // End collapse.
    ...
    public static function bulk_update_status($targetfield, $targetvalue, $enabled) {
        ...
        $sqltargetfield = $DB->sql_compare_text('targetfield') . ' = ' . $DB->sql_compare_text(':targetfield');
        $sqltargetvalue = $DB->sql_compare_text('targetvalue') . ' = ' . $DB->sql_compare_text(':targetvalue');
        $whereclause = $sqltargetfield . ' AND ' . $sqltargetvalue;

        // First check if any records exist.
        $count = $DB->count_records_select('local_profilefield_autofill_mapping', $whereclause, $params);
        ...
        // Update the records.
        $result = $DB->set_field_select('local_profilefield_autofill_mapping', 'enabled', $enabled, $whereclause, $params);
    ...
    public static function bulk_delete($targetfield, $targetvalue) {
        ...
        return $DB->delete_records_select('local_profilefield_autofill_mapping', $whereclause, $params);
    }
    ...
    private static function validate_csv_row($row, $rownum) {
        ...
        try {
            ...
            // Skip field validation if the database is not accessible. This allows the
            // import to proceed with basic validation only.
